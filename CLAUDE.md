...
- `scripts/test-consistency.sh` — 정합성 검사 회귀 fixture: H 마일스톤 배치 · 준비도 롤업 · I 문서 등재 대조 ·
  **J 계획 깊이**(사양 표·권한 표·`self:plan` 경계·도입 전/후 양쪽 · **본체 호출 경계**) ·
  **K 품질 명령 확정**(빈 명령 칸·미선택 양식 칸·절 이름/열 이름 개명의 fail-closed · **미끼 표**(순서만 바꾼 머리행)·
  **절 밖 표**의 우회·오탐 양방향 · 「해당 없음」 면제와 `/` 실값의 오탐 경계 · **전 행 면제** · 빈 칸 위장 7종 · 미선택 변형 3종 ·
  착수 전 휴면 · 상태 열 부재 · **셸 호출 경계**(엔진 부재·비정상 rc) · **배포 양식 그대로 물리기**(양식 드리프트 감시) ·
  **착수 건수 계수**(🔵 뿐 아니라 🟡·✅ 도 착수다) · 호출자가 건수를 안 주면 본체가 실패) (CI).
  **실행 단언 206건**(그중 J 112 · K 36) **+ 뮤테이션 자기검증 12** = 단언 218줄 (전체 출력 235줄 — 머리말·빈 줄 17 포함).
  **세는 기준을 함께 적는다** — `expect_*` 호출 지점은 185 이고, 거기에 루프 전개 +13(J·K35 5 · K15 6 · K16 2)과
  손으로 쓴 `ok`/`ng` 단언 20(R10 3 + J35 1 + J35-b 1 + J38 1 + K9 1 + K10 1 + 뮤테이션 12)을 더해 단언 줄이 218 이 된다.
  ...
  **뮤테이션 12 중 둘은 조기 종료 두 자리를 각각 `exit 0` 으로 되돌려 뮤턴트의 rc 를 먼저 단언한다** —
  한때 그 자리는 주석만 「되돌리면」이라 쓰고 **코드는 뮤테이션을 안 가했다**(2회전 별건 #334).
  **뮤테이션 12 중 셋은 K 몫이다** — 셸의 K 호출 블록 하나와 **`check-quality.py` 본체 두 곳**(면제 판정·미선택 양식 판정).
  K 의 **표 읽기 규약**(절 이름 앵커 · 행별 머리행)은 뮤테이션이 아니라 K11~K13 fixture 가 우회·오탐 양방향으로 잰다 —
  1회전에서 그 두 방향이 **둘 다 재현**됐고 반증이 확정한 자리다.
...
- `scripts/test-install-upgrade.sh` — `plugins/mdm/scripts/init-project.sh`(`mdm init`) 회귀 fixture (CI). **단언 53줄 = 케이스 51 + 뮤테이션 자기검증 2**
  ...
  **핀 없는 1.x CI 옆에 양식 사이드카**(#427) · **심은 STATUS 가 S0 을 가리키는가**(#479 — 첫 세션이 도입을 건너뛰지 않게) · **실패 경로**(가이드 쓰기 실패 · 매니페스트 부재 · 객체 아닌 settings 값 · settings 링크 · `.gitignore` 쓰기 실패 — 「완료」를 찍지 않고 멈추는가).
...
- `plugins/mdm/scripts/check-quality.py` — **정합성 검사 K 의 본체**. 제품의 `docs/spec/code-conventions.md` 에서
  **절 이름**(「품질 명령」)으로 절을 찾고 그 아래 `검사`·`명령` 앵커 표가 채워졌는지 본다 — 절 **번호**로 찾지 않는 것은
  골격이 바뀌어도 안 깨지게, 절 **이름**으로 좁히는 것은 문서의 다른 표를 잘못 판정하지 않기 위해서다.
  **열 번호는 행마다 그 행의 머리행에서 뽑는다** (첫 표 머리행을 모든 행에 쓰던 판은 순서만 바꾼 미끼 표로 통째로 뚫렸다 — 1회전 반증 확정).
  **착수한 요구사항(🔵·🟡·✅)이 있을 때만 판정한다** — 건수는 `check-consistency.sh` 가 세어 `--active` 로 넘기고,
  0 이면 쉬었다고 **말하고** 통과한다 (갓 설치한 저장소를 붉히지 않기 위해서다). 절 이름·열 이름을 바꾸거나 표를 지우면
  fail-closed 로 붉어지고, **전 행이 「해당 없음」이어도 침묵하지 않고 실패한다**(확정된 명령이 0 개다).
  python3 부재는 따로 보지 않는다 — 위쪽 `mdm-contract.py` 가 먼저 죽어 **도달 불가**임을 실측했다(F6 반증).
  **적힌 명령이 실제로 도는지는 보지 않는다** — 그것은 `mdm contract verify` 의 실행 증거와 제품 CI 몫이고,
  이 검사는 「명령이 확정돼 있는가」까지다. 그 한계를 문서에도 약속이 아니라 그대로 적는다.
- `plugins/mdm/scripts/mdm_md.py` — J·K 가 **함께 쓰는** 마크다운 표 리더. 리더를 하나 더 만들지 않는다 —
  같은 결함 계열을 표 리더 다섯 개에서 밟은 뒤의 결정이고, K 를 넣을 때 복제 대신 여기로 뽑았다.
  이 파일을 고치면 J(112)와 K(36)가 함께 붉어져야 한다.

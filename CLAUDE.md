...
- `scripts/test-consistency.sh` — 정합성 검사 회귀 fixture: H 마일스톤 배치 · 준비도 롤업 · I 문서 등재 대조 ·
  **J 계획 깊이**(사양 표·권한 표·`self:plan` 경계·도입 전/후 양쪽) (CI).
  **실행 단언 157건**(그중 J 99) **+ 뮤테이션 자기검증 7** = 단언 164줄 (전체 출력 179줄 — 머리말·빈 줄 15 포함).
  **세는 기준을 함께 적는다** — `expect_*` 호출 지점은 147 이고, 거기에 루프 전개 +7 과
  손으로 쓴 `ok`/`ng` 단언 10(R10 3 + 뮤테이션 7)을 더해 단언 줄이 164 가 된다.
  재는 법: 단언 총계 `grep -cE '^  (통과|실패)  '` · J `grep -cE '^  (통과|실패)  J[0-9]'` ·
  뮤테이션은 「뮤테이션 자기검증」 줄 이후.
  ...
  **뮤테이션 7 중 둘은 조기 종료 두 자리를 각각 `exit 0` 으로 되돌려 뮤턴트의 rc 를 먼저 단언한다** —

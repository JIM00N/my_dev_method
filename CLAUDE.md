...
1. **변경 후 커밋 전에 `/kit-review`를 돌린다.** 코드리뷰는 서브에이전트가 한다 —
   ...
   **명령 표기를 바꾸는 것으로는** 못 피하고(`sh -c`·서브셸·`$(...)`·백틱·alias·`nohup`·`make` 등 17경로 실측),
   `<pathspec>`·`-a`·`--amend` 등 부분·대체 인덱스 모드도 막힌다. 도장 스크립트가 없거나 실행 권한이 없어도 **차단**한다(fail-closed).
...
2-1. **커밋 정책: 승인 모드.** 리뷰를 통과시킨 뒤 **묻고 답을 받아야** 커밋한다. 물을 때 무엇이 바뀌었는지·
   리뷰 결과·남은 지적을 함께 준다. push·태그·릴리스는 모드와 무관하게 승인받는다.

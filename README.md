...
Claude Code 플러그인으로 설치하는 [dev-kit](plugins/mdm)입니다. 커맨드·훅·리뷰 에이전트·Bash/Python 검사 엔진은 **플러그인이 제공**하고, 제품 저장소에는 `/mdm:init`이 지시서와 문서 양식만 심습니다. 기존 계획 문서를 가져올 수 있고, 계획이 없으면 `/mdm:plan`으로 작성합니다.

**처음 읽는 순서:** 이 README → [하네스 구성과 소스 안내](guides/harness-architecture.md) → [요구사항 하나로 따라가는 동작 예시](guides/harness-walkthrough.md).

## V2.0.0에서 달라진 것

키트가 **Claude Code 플러그인**이 됐습니다. 1.x는 `.claude/` 전체를 제품에 복사했고, 그 결과 커맨드·훅·엔진이 제품마다 다른 판으로 갈라졌습니다.

- **플러그인 `mdm@my-dev-method`:** 커맨드 `/mdm:adopt`·`/mdm:ready`·`/mdm:review` 등, 서브에이전트 `mdm:code-review`·`mdm:error-learning`, 훅 3개, 검사 엔진이 플러그인에서 옵니다. `/plugin update`로 한 번에 올립니다.
- **`/mdm:init`:** 제품에는 `CLAUDE.md`·`AGENTS.md`·`docs/`·제품 CI 양식만 심고, `.claude/settings.json`에 플러그인을 프로젝트 범위로 등록합니다. 1.x 잔재는 내용 해시로 분류해 알리며 기본으로 건드리지 않습니다.
- **`mdm` 런처:** 문서와 커맨드의 엔진 호출이 `mdm check`·`mdm final`·`mdm contract …`·`mdm ops …`·`mdm report …`로 통일됐습니다. 제품 루트는 `MDM_PROJECT_ROOT` → `CLAUDE_PROJECT_DIR` → git 루트 → 현재 디렉토리 순으로 찾습니다.
- **제품 CI:** 원본 저장소를 `MDM_KIT_REF` 핀으로 받아 엔진을 실행합니다. `mdm doctor`가 핀과 플러그인 버전을 대조합니다.

### V1.0.0에서 달라진 것

계약 근거·실행 증거·변경 운영 기능을 V1.0.0으로 묶었습니다.
...
- **제품 저장소의 최종 검사:** `mdm final`와 제품 CI 양식으로 정합성·증거·인계를 함께 확인합니다.
...
| 구성 | 역할 | 직접 확인할 자료 |
|---|---|---|
| 지시서·가이드 | 지금 읽을 문서, 단계, 역할과 결정 절차를 안내 | [CLAUDE.md](plugins/mdm/templates/CLAUDE.md), [단계별 가이드](plugins/mdm/templates/docs/guides/index.md) |
| 프로젝트 문서 | 상류·설계·Story·현재 상태의 정본 보관 | [문서 지도](plugins/mdm/templates/docs/MOC.md), [요구사항 추적표](plugins/mdm/templates/docs/spec/source-map.md) |
| 커맨드·에이전트 | 도입·계획·준비·리뷰·종료·오류 학습 수행 | [커맨드와 에이전트 구성](plugins/mdm/README.md) |
| 검사기·증거 엔진 | 문서 구조, 입력 변경, 준비·실행 근거 검사 | [계약 근거 가이드](plugins/mdm/templates/docs/guides/contract-evidence.md) |
| 훅·CI | 도구 사용 시 검사 연결, 종료·통합 시 최종 확인 | [훅 등록](plugins/mdm/hooks/hooks.json), [제품 CI 양식](plugins/mdm/templates/.github/workflows/mdm-check.yml) |
| 플러그인 매니페스트 | 이름 `mdm`·버전·마켓플레이스 등록 | [plugin.json](plugins/mdm/.claude-plugin/plugin.json), [marketplace.json](.claude-plugin/marketplace.json) |
...
Claude Code(플러그인 지원판)에서 커맨드·훅·서브에이전트를 사용하는 구성이 기본입니다. Bash, Git, Python 3.7 이상, `mktemp`, `jq`가 필요합니다. `jq`가 없으면 의존성·비밀값 guard 훅은 경고 후 통과하므로 설치 여부를 확인하세요.

다른 에이전트에서도 `AGENTS.md`로 규칙을 읽고 검사 엔진을 직접 실행할 수 있습니다(원본 저장소를 받아 `plugins/mdm/bin/mdm`을 부릅니다). Claude Code용 훅·커맨드·서브에이전트는 Claude Code에서만 동작합니다.
...
에이전트가 계획 문서를 찾고 상류 스냅샷·요구사항 매핑을 준비합니다. 문서가 없으면 `/mdm:plan`으로 연결합니다. 이미 결정된 내용은 인용하고, 부족한 상태·권한·스택·검증 기준을 확인합니다. 상세 절차는 [S0 도입](plugins/mdm/templates/docs/guides/S0-adopt.md)이 정본입니다.
...
이 명령은 정합성·등록된 증거·인계를 검사합니다. 프로젝트 테스트를 자동으로 선택해 실행하지는 않습니다. 테스트 실행을 증거로 남기는 `verify`와 인계 작성 방법은 [계약 근거](plugins/mdm/templates/docs/guides/contract-evidence.md), [변경 운영](plugins/mdm/templates/docs/guides/operating-loop.md)을 참고하세요.
...
├── .claude/settings.json       플러그인 mdm@my-dev-method 등록 (커맨드·에이전트·훅·엔진은 플러그인이 제공 — 복사되지 않는다)
├── .github/workflows/         제품 최종 검사 CI (MDM_KIT_REF 핀으로 원본 저장소의 엔진을 받는다)
└── docs/
...
Lite / Standard / Full [프로파일](plugins/mdm/templates/docs/guides/profiles.md)로 프로젝트에 맞는 절차량을 선택합니다. Lite에서도 계약·테스트·현재 증거·인계 확인은 유지합니다.

## 기존 버전에서 업그레이드

```text
/plugin update mdm@my-dev-method     # 커맨드·훅·엔진 (세션 재시작)
/mdm:init --upgrade                  # 제품 저장소에서 — 키트 소유 문서만 교체
```

설치기는 키트 소유 문서를 교체하고 프로젝트 기록은 보존합니다. `CLAUDE.md.dev-kit-new`가 생겼다면 프로젝트명·§6 고유 규칙과 수동 병합합니다. 제품 CI에 `MDM_KIT_REF` 핀이 있으면 새 버전으로 올리고, 없으면(1.x 양식) 설치기가 옆에 둔 `mdm-check.yml.dev-kit-new`로 교체합니다. 제품 CI 파일 자체는 기존 파일이 있으면 보존합니다.
...
| 플러그인 구성·소유권·1.x 이관 | [dev-kit 플러그인 안내](plugins/mdm/README.md) |
| JSON 형식과 계약·실행 명령 | [계약 근거와 실행 증거](plugins/mdm/templates/docs/guides/contract-evidence.md) |
| 영향별 리뷰·동기화·인계·CI 진단 | [변경 운영 가이드](plugins/mdm/templates/docs/guides/operating-loop.md) |
...
이 저장소의 [CI](.github/workflows/docs-check.yml)는 문서 참조와 플러그인 매니페스트, 리뷰 게이트, 정합성, 리포트, 문서 검사기, 설치·업그레이드, 플러그인 훅, 계약·운영 회귀를 검사합니다. 제품 저장소용 CI와 구분됩니다.

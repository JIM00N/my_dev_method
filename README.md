...
| **답이 없는 칸을 남긴 채 구현을 시작한다** → 에이전트가 지어낸다 | `/ready` — Story 슬롯 12칸을 AI가 채우고 **갈리는 것만** 질문 → 미달이면 못 엽니다 | **스크립트** |
...
| **보안 구멍과 부작용이 정상 경로만 통과한 채 지나간다** — 남의 것 접근, 클라이언트 값 신뢰, 범위 없는 삭제, 실패 시 열리는 권한 | 리뷰 4축 16검사 — 규칙 위반 · 보안 · 의도치 않은 동작 · 검증. 기준은 `architecture.md` 3·4절과 `domain.md` 4·5절이 이미 적어 둔 대비 | **서브에이전트** |
...
```text
dev-kit v0.5.0 → /path/to/제품저장소 (install)

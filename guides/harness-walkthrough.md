...
이 문서는 하네스가 남기는 기록과 검사 시점을 설명하는 **가상 제품 예시**입니다. 실행 가능한 완성 제품이나 실전 효과 측정 결과는 아닙니다. 실제 명령 인자·JSON 양식은 [계약 근거 가이드](../templates/dev-kit/docs/guides/contract-evidence.md)와 [변경 운영 가이드](../templates/dev-kit/docs/guides/operating-loop.md)를 따릅니다.
...
등록 방법은 [Story 등록 예제](../templates/dev-kit/docs/guides/contract-evidence.md)를 참고합니다. `mdm-ops.py plan ST-001`은 등록된 영향에 따라 읽을 문서·비교 항목을 안내합니다.
...
동기화 중 프로세스가 강제 종료되어 복구 기록이 남으면 운영 검사는 실패합니다. 복구는 이전 상태를 되살리는 절차이며, 새 변경은 다시 preview부터 검토합니다. 구체적인 복구 방법은 [변경 운영 가이드](../templates/dev-kit/docs/guides/operating-loop.md)를 따릅니다.
...
handoff는 이 내용과 현재 파일 집합·해시를 연결합니다. 같은 날짜라도 STATUS나 추적 대상 파일이 바뀌면 새 인계가 필요합니다. 마지막에 제품 저장소에서 `bash .claude/scripts/mdm-check.sh`를 실행합니다.

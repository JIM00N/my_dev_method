...
# 제품 루트와 키트 표식은 훅 공용 lib-root.sh 가 정한다 — MDM_PROJECT_ROOT → CLAUDE_PROJECT_DIR → PWD 에서
# 출발해 git 최상위까지 위로 docs/status/STATUS.md 를 찾는다. 플러그인 훅은 켜진 **모든** 저장소에서 돌므로
# 표식이 없으면 판정하지 않고 통과한다 — 1.x 에서 훅 파일이 키트 프로젝트에만 있던 것과 같은 범위다.
# 표식을 지우면 훅이 꺼진다 — 옛 방식에서 훅 파일을 지우면 꺼지던 것과 같은 신뢰 수준이다 (플러그인 README 「훅이 판정하는 저장소」).
. "$(dirname "$0")/lib-root.sh"
mdm_find_root || exit 0

...
# 엔진은 플러그인 안에 있다 — 자기 경로로는 제품을 못 찾는다. 제품 루트는 호출 환경이 정한다:
# MDM_PROJECT_ROOT → CLAUDE_PROJECT_DIR → git 루트 → 현재 디렉토리 (plugins/mdm/scripts/mdm_env.py 머리말이 정본).
# 셸이 정한 루트를 MDM_PROJECT_ROOT 로 내보내 파이썬 본체(check-plan.py·mdm-contract.py·mdm-ops.py)와 같은 제품을 보게 한다.
...
ROOT="$(cd "$ROOT" 2>/dev/null && pwd)" || { printf '실패  제품 루트를 열 수 없다: %s\n' "${MDM_PROJECT_ROOT:-${CLAUDE_PROJECT_DIR:-$PWD}}"; exit 1; }
...
finish() {
  ...
  if [ "$fail" = 0 ] && [ "$warn" = 0 ]; then
    ...
    echo "유형별 해결 경로: /mdm:review 1단계 표. 사용자 판정이 필요하면 STATUS 의 막힌 것에 ⛔ 로 올리고 멈춘다."
    exit 1
  fi
  exit 0
}
...
# 검사 본체는 **같은 디렉토리의 `check-plan.py`** 다. 이 자리는 그것을 부르고 결과를 흘려보낸다.
#
...
J_RAN=0
J_PLAN_ROW=0
J_PY="$ENGINE/check-plan.py"
J_STATE=""
if [ ! -f "$J_PY" ]; then
  bad "엔진에 check-plan.py 가 없다 ($J_PY) — 계획 깊이 검사(J)를 돌릴 수 없다. 플러그인 설치가 깨졌다: /plugin 에서 mdm 을 다시 설치한다"
  ...
  if [ -z "$J_STATE" ]; then
    ...
    case "$J_RC" in
      ...
      1) fail=1 ;;
      2) warn=1 ;;
      *) bad "계획 깊이 검사(J)가 비정상 종료했다 (rc=$J_RC) — check-plan.py 를 직접 돌려 원인을 본다" ;;
    esac
    ...
    [ "$J_STATE_OK" = 2 ] || bad "계획 깊이 검사(J)의 상태 파일이 형식을 벗어났다 (ran·plan_row 둘을 못 읽었다) — check-plan.py 를 직접 돌려 원인을 본다"
  fi
fi
...
if ! python3 "$ENGINE/mdm-contract.py" "${MODEL_ARGS[@]}"; then
...
python3 "$ENGINE/mdm-ops.py" refs || bad "MDM_REFERENCE — 제품 문서의 참조를 확인한다"

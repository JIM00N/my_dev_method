...
# 제품 루트는 호출 환경이 정한다 (mdm_env.py 머리말) — 엔진은 플러그인 안에 있어 자기 경로로는 못 찾는다.
ROOT = mdm_env.project_root()
ENGINE = mdm_env.ENGINE
...
def story_inputs(sid, data=None):
    ...
    return {'schema_version': 1, 'story': sid, 'definition': story,
            ...
            # 정책 파일은 제품이 아니라 엔진(플러그인)에 있다 — path() 는 제품 상대 경로만 받으므로 직접 읽는다.
            'policy': {name: hashlib.sha256((ENGINE / name).read_bytes()).hexdigest()

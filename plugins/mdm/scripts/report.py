...
"""문서를 사람이 보기 쉬운 HTML 한 장으로 만든다.
    ...
    mdm report <종류> [--open]      (플러그인 bin/mdm 런처 · 직접: python3 <플러그인>/scripts/report.py)
...
def check_output(initializing=False):
    sh = os.path.join(ENGINE, "check-consistency.sh")
    if not os.path.exists(sh):
        return None, ""
    try:
        env = dict(os.environ, MDM_PROJECT_ROOT=ROOT)
        p = subprocess.run(["bash", sh] + (["--init"] if initializing else []), capture_output=True, text=True, timeout=120, cwd=ROOT, env=env)
...
def build(kind):
    ...
    if os.path.isfile(os.path.join(ROOT, "docs", "meta", "project.json")):
        ...
        spec = importlib.util.spec_from_file_location("mdm_contract", os.path.join(ENGINE, "mdm-contract.py"))
...
<header><h1>%s</h1><div class="sub">%s · 생성 %s · <code>mdm report %s</code></div></header>
%s

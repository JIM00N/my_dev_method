...
**이 가이드가 운영 준비·완료 판정의 정본이다.** 적용 엔진은 `mdm-contract.py`,
source-map 공통 reader는 `mdm_model.py`다. 기존 A~K 중 J·참조·등재 검사는 유지한다.
...
| docs/meta/project.json | 스키마 1, 도입 완료 표시 | adopt가 최초 생성. 설치기(`/mdm:init`)·업그레이드는 손대지 않음 |
...
**작업 중 전체 정합성은 위 명령, 종료·통합은 인계 후 `mdm final`를 사용한다** (`docs/guides/operating-loop.md`).
--scope ST-001은 그 Story가 연결한 요구사항의 증거 검사만
선택한다. 구조 검사와 기존 A~K는 전체에 적용되며, 같은 요구사항을 나눈 Story는 함께 준비되어야 한다.

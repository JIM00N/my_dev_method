...
엔진은 `mdm_operations.py`, 실행 진입점은 `mdm-ops.py`다.
...
`mdm contract adopt`와 운영 check는 상류에서 다시 추출한 전체 ID와 처리 목록·매핑을 대조한다.
...
```bash
mdm ops handoff --note .tmp/handoff.json
mdm final
```
...
Stop 훅은 도입 후 이 검사를 쓰며 반복 차단 방지는 유지한다. 최종 통합의 `mdm final`에는 그 예외가 없다.
작업 도중에는 `mdm check`, 종료·CI에서는 `mdm final`를 쓴다.
...
```bash
mdm ops freshness --failed '원격 조회 실패 사유'
```
...
```bash
mdm ops sync-preview --bundle .tmp/sync.json
mdm ops sync-apply --bundle .tmp/sync.json --basis '<preview의 basis>'
mdm ops freshness
```
...
남은 docs/meta/.mdm-write.lock 빈 디렉터리를 제거하고 `mdm ops sync-recover`로 복원한다.
...
설치기(`/mdm:init`)는 `.github/workflows/mdm-check.yml`을 없을 때만 생성한다. 기존 파일은 보존한다.
...
mdm ops doctor
mdm ops doctor --run
mdm ops doctor --github owner/repo --branch main
```
...
`mdm ops pilot --record .tmp/pilot.json`은 측정값을 검증하고 질문·영향 후보의 유효 비율을 계산한다.

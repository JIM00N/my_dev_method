...
**강제 장치**(훅·스크립트·커맨드·서브에이전트)는 문서가 아니라 `mdm` 플러그인이 제공한다 — 이 저장소에는 복사되지 않는다. 목록은 플러그인 README — 제품에는 복사되지 않는다. 위치는 `mdm doctor` 의 `engine` 필드(그 상위 폴더의 `README.md`), 원본은 `github.com/JIM00N/my_dev_method` 의 `plugins/mdm/README.md`.
...
- 그래서 새 프로젝트를 시작할 때 `guides/`는 그대로 복사하고 나머지는 비운다. 강제 장치는 복사하지 않는다 — 플러그인이 맡는다.
...
사이클·ADR·Story의 등재는 도입 후 `mdm check` 검사 I가 대조한다 —

...
def handoff_check(E):
    ...
    require(E, receipt and receipt.get('files') == work_snapshot(E), 'MDM_HANDOFF', '이번 변경에 대응하는 인계 기록이 없다. STATUS 갱신 후 handoff를 실행한다')
    return receipt
...
def doctor(E, repo, branch, run=False):
    ...
    result = {'runtime': 'python3', 'scripts': {n: E.path('.claude/scripts/' + n, False).is_file() for n in scripts},
              'adopted': False,
    ...
    if run:
        p = subprocess.run(['bash', str(E.path('.claude/scripts/mdm-check.sh'))], cwd=str(E.ROOT), capture_output=True, text=True)

...
`CLAUDE.md`의 규칙 중 4·9·12는 `.claude/hooks/`가 기계적으로 강제하고, 규칙 11은 `code-review` 서브에이전트(`/review` 2단계)가 잡는다. 훅이 막으면 우회하지 말고 막힌 이유를 해결한다. 이 강제 장치는 Claude Code에서만 동작하므로, 다른 에이전트로 작업할 때는 해당 규칙을 스스로 지켜야 한다.

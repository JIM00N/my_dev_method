...
**버전: 1.0.0** (각 배포본의 버전은 `CLAUDE.md` 첫 줄의 `<!-- dev-kit v… -->` 스탬프로 확인한다)
...
| **`python3` (3.7+)** | **정합성 검사 J**(`.claude/scripts/check-plan.py`) · 계약 근거 엔진 · 리포트(`report.py`) | **검사 J 가 실패한다 — 건너뛰지 않는다** |
...
상류 전체 요구사항 대조·위험별 의미 리뷰·현재 변경에 대응하는 인계·동기화 복구·제품 CI는
`docs/guides/operating-loop.md`를 따릅니다. 업그레이드 후 requirements 목록과 비교 근거를 먼저 이관합니다.
설치기는 제품 CI 파일을 없을 때만 생성합니다. 실제 활성화는 `mdm-ops.py doctor`로 구분해 확인합니다.
